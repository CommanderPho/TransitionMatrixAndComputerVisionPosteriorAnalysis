...
% 8/4/15:   Minor tweaks to layout, fontsizes. Improved visualization of
%           Houghpeaks plot.
% 4/12/16   Minor bug fix at histogram title.
% 5/16/16   Disabled auto-conversion to double of loaded images.
% 8/22/16   Implemented auto-restriction of Canny threshold to (0, 1).
% 6/29/20   Fixed tag of RhoRes slider.
% 11/10/20  Nested all subfunctions to share variables. Also fixed spacing.
%
...
[allTooltips, autoSens, ...
    ...
    edgeDir, edgeType, extDir, extType, graythreshVal, ...
    histax, histtitle, houghAx, houghLinesOpts, ...
...
commentBox = uicontrol(commentPanel, 'style', 'listbox', ...
    ...
    'foregroundcolor', 'k', 'fontsize', defaultFontsize, 'max', 10, 'min', 1, ...
...
% BEGIN NESTED SUBFUNCTIONS
    ...
    function overlayColor = changeOverlayColor(varargin)
        ...
        if islogical(img)
            ...
            currOverlay = findall(parentFigure, 'tag', 'opaqueOverlay');
    ...
    function img = getFile(varargin)
        [img, cmap, original.fname, ~, userCanceled] = getNewImage(false);
        if userCanceled
            return
        end
        lastCommand = '';
    ...
    function processRequest(varargin)
        ...
        switch reqObjTag %Alphabetical listing
            case 'ActiveContour'
                ...
                %set(parentFigure, 'pointer', 'arrow');return
            case 'calcColorMasks'
            ...
            case 'makeAdaptive'
                if get(gcbo, 'value') == 1
                    segType = 'adaptiveThreshold';
                else
                    segType = 'Thresholding';
    ...
    function resetGraythreshVal(varargin)
        if strcmp(get(parentFigure, 'selectiontype'), 'alt')
    ...
    function img = segment(segType, varargin)
        ...
        switch segType
            case 'Edge'
                ...
                switch requestedEdge
                    ...
                    case {'LOG', 'Canny'}
                        ...
                        if strcmp(requestedEdge, 'Canny') && ~isempty(thresh) && (isequal(thresh, 0) || isequal(thresh, 1))
                            thresh = max(min(thresh, 1-eps), 0+eps);
                            beep
                            fprintf('NOTE:\nFor Canny edge detection, \nthreshold must be specified on (0, 1);\nit has been adjusted in segmentImage.\nYou must ensure that in your code, \nit is on that [exclusive] range.\n')
                        end
            ...
            case 'adaptiveThreshold'
                ...
                throwComment('ADAPTIVE THRESHOLDING', 0, 1);
                img = imbinarize(img, 'adaptive', 'sensitivity', threshVal);
                %img = imbinarize(img, 'adaptive');
                %img = img > threshVal;
                %lastCommand = sprintf('img = imbinarize(img, ''adaptive'');');
                lastCommand = sprintf('img = imbinarize(img, ''adaptive'', ''sensitivity'', %0.2f)', threshVal);
            case 'multithresh'
                if isrgb(img)
                    usingGray = true;
                    img = original.grayversion;
            ...
            case 'Hough Line/Circle'
                if ~islogical(img)
                    ...
                    set(houghAx, 'fontsize', defaultFontsize);
                    lines = [];
                    if ~isempty(P)
                        % 						home
            ...
            case 'calcColorMasks'
                ...
                delete(findall(parentFigure, 'tag', 'colorMaskAx'));
                ind = 1;
                for jjsub = 1:nCols
                    for iisub = 1:nRows
        ...
        switch requestedPanel{1} %Alphabetical listing
            ...
            case 'Edge'
                ...
                uicontrol('parent', parent, 'style', 'text', ...
                    ...
                    'fontsize', defaultFontsize, 'foregroundcolor', txtc, ...
            ...
            case 'Hough Line/Circle'
                ...
                rhoRes = ...
                    ...
                    'value', 1, 'callback', @processRequest, ...
					'sliderstep',[1/90 5/90],'tag','RhoResSldr',...
                ...
                houghPeakOpts(1) = uicontrol('parent', parent, 'style', 'edit', ...
                    'string', sprintf('%0.1f', 0.5*max(original.img(:))), ...
                ...
                houghAx = axes('parent', parent, 'units', 'normalized', 'pos', [0.085+0.05 0.19+0.01 0.85-0.025 0.125-0.01], ...
                    'fontsize', defaultFontsize, 'visible', 'off');
            case 'Regional/Extended Min/Max'
    ...
    function togglePointer(pType)
        if nargin == 0 || isempty(pType)
            if strcmp(get(parentFigure, 'pointer'), 'arrow')
                set(parentFigure, 'pointer', 'watch');
            else
                set(parentFigure, 'pointer', 'arrow');
            end
        else
            set(parentFigure, 'pointer', pType);
        end
        drawnow;
    end %togglePointer
    ...
    function draggableBrief(h, endfcn)
        ...
        function click_object(obj, eventdata)
            % obj here is the object to be dragged and parentFigure is the object's parent
            ...
            set(parentFigure, 'WindowButtonDownFcn', {@activate_movefcn, obj});
            set(parentFigure, 'WindowButtonUpFcn', {@deactivate_movefcn, obj});
            activate_movefcn(parentFigure, eventdata, obj);
        end
        ...
        function movefcn(~, ~, h)
            threshvalIndicator = findall(parentFigure, 'tag', 'threshvalIndicator');
            % obj here is the *figure* containing the object
